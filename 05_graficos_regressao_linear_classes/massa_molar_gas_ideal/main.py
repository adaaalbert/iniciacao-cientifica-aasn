...
dados = np.genfromtxt((replace_comma_to_dot(text) for text in
                       open('dados.csv')),
...
regressao = linregress(valores_densidade.magnitude,
                       pressao_sobre_densidade.magnitude)
massa_molar = gas_constant * temperature / (regressao.intercept *
                                            ureg('bar / (g/l)'))
